...
FGA.setBreadcrumbs = function ( path ) {
	...
	const htmFolders = folders.slice( 1 ).map( ( folder, i ) =>
		`<a href=JavaScript:FGA.fetchTree("${ folders.slice( 0, i + 2 ).join( "/" ) }"); >${ folder}</a> &raquo; `
	).join( "" );
...
FGA.getFolders = function (items) {
	...
	const htm = items.filter( item => item.type === "tree" &&
		!COR.ignoreFolders.includes( item.name ) ).map( item => `
...
FGA.getFiles = function ( items = [] ) {
	...
	const htm = items.filter( item => item.type === "blob" &&
		!COR.ignoreFiles.includes( item.name ) ).map( item => {

			//console.log( 'item', item );

			FGA.urlGitlabSource = `https://gitlab.com/${COR.group}/${COR.repo
